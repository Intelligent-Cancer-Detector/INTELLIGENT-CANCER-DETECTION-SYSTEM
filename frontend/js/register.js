const SUPABASE_URL ="https://supabase.com/dashboard/project/tgrrmzusqjzzvhevmmbt";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRncnJtenVzcWp6enZoZXZtbWJ0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUyOTg4NDUsImV4cCI6MjA5MDg3NDg0NX0.nmD117ohEA-pMV4YnNluPxJGT4N-HFJxPaRRyGFyyks";
...
function applyAISuggestion() {
    document.getElementById('hospitalName').value = document.getElementById('aiSuggestText').innerHTML;
    document.getElementById('aiSuggest').classList.remove('show');
}
...
function checkPasswordStrength() {
    ...
    if (strength < 25) { fill.style.background = '#ef4444'; text.innerText = 'Weak password'; }
    else if (strength < 50) { fill.style.background = '#f59e0b'; text.innerText = 'Fair password'; }
    else if (strength < 75) { fill.style.background = '#3b82f6'; text.innerText = 'Good password'; }
    else { fill.style.background = '#10b981'; text.innerText = 'Strong password'; }
}
...
function validateAll() {
    ...
    if (!hospitalEmail.value) { missingFields.push('Institution Email'); highlightField(hospitalEmail); }
    if (!hospitalName.value) { missingFields.push('Institution Name'); highlightField(hospitalName); }
    if (!hospitalType.value) { missingFields.push('Institution Type'); highlightField(hospitalType); }
    if (!address.value) { missingFields.push('Street Address'); highlightField(address); }
    if (!city.value) { missingFields.push('City'); highlightField(city); }
    if (!phone.value) { missingFields.push('Main Phone'); highlightField(phone); }
    if (!fullName.value) { missingFields.push('Full Name'); highlightField(fullName); }
    if (!designation.value) { missingFields.push('Designation'); highlightField(designation); }
    if (!department.value) { missingFields.push('Department'); highlightField(department); }
    if (!adminEmail.value) { missingFields.push('Work Email'); highlightField(adminEmail); }
    if (!adminPhone.value) { missingFields.push('Direct Phone'); highlightField(adminPhone); }
    if (!username.value) { missingFields.push('Username'); highlightField(username); }
    if (!password.value) { missingFields.push('Password'); highlightField(password); }
    ...
    if (!agreeTerms.checked) { missingFields.push('Accept Terms & Conditions'); }
    if (!agreePrivacy.checked) { missingFields.push('Accept Privacy Policy'); }
...
document.addEventListener('keydown', function(e) {
    if (e.ctrlKey && e.shiftKey && e.key === 'E') {
        ...
        document.getElementById('fullName').value = 'Dr. Wycliff Nthiga';
        document.getElementById('designation').value = 'medical-director';
        document.getElementById('department').value = 'administration';
        document.getElementById('adminEmail').value = 'wycliffr254@gmail.com';
        document.getElementById('adminPhone').value = '+254700000000';
        document.getElementById('username').value = 'drwycliff';
        document.getElementById('password').value = 'Test12345678';
        document.getElementById('confirmPassword').value = 'Test12345678';
        document.getElementById('agreeTerms').checked = true;
        document.getElementById('agreePrivacy').checked = true;
        
        const allFields = ['hospitalEmail', 'hospitalName', 'hospitalType', 'address', 'city', 'phone', 
                          'fullName', 'designation', 'department', 'adminEmail', 'adminPhone', 'username', 'password'];
