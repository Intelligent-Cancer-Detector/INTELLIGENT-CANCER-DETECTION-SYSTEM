...
const cancers = [
  {
    name: "Lung Cancer",
    keys: ["cough", "chest", "breath"],
    questions: [
      "Cough persistent for 3+ weeks?",
      "Blood in sputum?",
      "History of smoking?",
    ],
  },
  {
    name: "Breast Cancer",
    keys: ["lump", "dimpling", "nipple"],
    questions: [
      "Is the lump hard and painless?",
  ...
  },
  {
    ...
    keys: ["pelvic", "discharge", "sex", "bleeding"],
    questions: ["Post-coital bleeding?", "HPV history?", "Abnormal discharge?"],
  },
  {
    name: "Brain Cancer",
    keys: ["seizure", "vision", "headache"],
    questions: [
      "Morning headaches?",
  ...
  {
    name: "Skin Cancer",
    keys: ["mole", "lesion", "border"],
    questions: [
      "Irregular mole borders?",
      "Multi-colored lesion?",
      "Itching/Bleeding?",
    ],
  },
  {
    name: "Pancreatic Cancer",
    keys: ["back pain", "diabetes", "pale stool"],
    questions: [
      "New onset adult diabetes?",
  ...
  {
    name: "Eye Cancer",
    keys: ["blurred", "shadows", "iris", "bulging"],
    questions: [
...
window.generateFollowUps = () => {
  ...
  if (!found)
    qList.innerHTML =
      "<p style='text-align:center; color:#64748b;'>No specific triggers found. Proceeding to risk calculation.</p>";
  window.goToStep(3);
};
...
window.runFinalAnalysis = function () {
  ...
  cancers.forEach((c) => {
    let matches = 0;
    c.keys.forEach((k) => {
      if (symptomsText.toLowerCase().includes(k)) matches++;
    });
    let sympScore = (matches / c.keys.length) * 40;

    let qTotal = 0;
    const selects = document.querySelectorAll(
      `.q-select[data-cancer="${c.name}"]`,
    );
  ...
  document.getElementById("resIdDisplay").textContent =
    `Patient ID: #${generatedPatientId}`;

  const badge = document.getElementById("statusBadge");
  badge.textContent =
    maxRisk > 70 ? "HIGH RISK" : maxRisk > 35 ? "MEDIUM RISK" : "LOW RISK";
